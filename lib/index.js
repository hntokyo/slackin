...
import log from './log'
import blockInvite from './block-invite'
    ...
    const captcha_callback = (err, resp) => {
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          if (blockInvite(email, { blocked_ip_addresses, blocked_email_domains, blocked_email_addresses })) {
            //make the troll wait 2 seconds and then lie about why.
            setTimeout(function(){
                res
                .status(400)
                .json({ msg: "Slack API endpoint timeout.  Please try again later." })
              }, 2000);
          }else{
